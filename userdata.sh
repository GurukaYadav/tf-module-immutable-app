...
sed -i -e "s/ENV/${ENV}/" -e "s/DOCDB_ENDPOINT/${DOCDB_ENDPOINT}/" -e "/java/ s/MEM/$MEM/g" -e "s/REDIS_ENDPOINT/${REDIS_ENDPOINT}/" -e "s/RDS_ENDPOINT/${RDS_ENDPOINT}/" -e "s/DOCDB_USER/${DOCDB_USER}/" -e  "s/RABBITMQ_USER_PASSWORD/${RABBITMQ_USER_PASSWORD}/"  -e  "s/DOCDB_PASS/${DOCDB_PASS}/"
